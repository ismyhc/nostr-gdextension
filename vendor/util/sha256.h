...
static inline void sha256_endian_reverse64(uint64_t input, uint8_t *output) {
...
static inline uint32_t sha256_endian_read32(uint8_t *input) {
    uint32_t output = 0;
    output |= (uint32_t)(input[0] << 24);
    output |= (uint32_t)(input[1] << 16);
    output |= (uint32_t)(input[2] << 8);
    output |= (uint32_t)(input[3] << 0);

    return output;
}

static inline void sha256_endian_reverse32(uint32_t input, uint8_t *output) {
...
static inline uint32_t sha256_ror(uint32_t input, uint32_t by) {
    return (input >> by) | (((input & ((1u << by) - 1u))) << (32u - by));
}

static inline void sha256(const void *data, uint64_t len, void *output) {
    uint8_t padding[80];
    uint64_t current = (len + 1u) % 64u;
    uint64_t needed = (64u + 56u - current) % 64u;
    uint64_t extra = needed + 9u;
    uint64_t total = len + extra;

    for (int i = 1; i < 80; i++)
        padding[i] = 0;
    padding[0] = 0x80;
    sha256_endian_reverse64(len * 8u, padding + total - len - 8u);

    uint32_t v[8];
    for (int i = 0; i < 8; i++)
        v[i] = sha256_initial_h[i];

    for (uint64_t cursor = 0; cursor * 64u < total; cursor++) {
        uint32_t t[8];
        for (int i = 0; i < 8; i++)
            t[i] = v[i];

        uint32_t w[64];
        if (cursor * 64u + 64u <= len) {
            for (int j = 0; j < 16; j++) {
                w[j] = sha256_endian_read32(
                    (uint8_t *)data + cursor * 64u + (uint64_t)j * 4u);
            }
        } else {
            if (cursor * 64u < len) {
                uint64_t size = len - cursor * 64u;
                if (size > 0) memcpy(w, (uint8_t *)data + cursor * 64u, (size_t)size);
                memcpy((uint8_t *)w + size, padding, 64u - (size_t)size);
            } else {
                uint64_t off = (cursor * 64u - len) % 64u;
                memcpy((uint8_t *)w, padding + off, 64u);
            }
    ...
    for (int i = 0; i < 8; i++)
        sha256_endian_reverse32(v[i], (uint8_t *)output + i * 4u);
}
